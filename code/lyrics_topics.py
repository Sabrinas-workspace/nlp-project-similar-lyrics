...
def query_get_song_recommendation(songtitle, artist, root):
    ...
    if len(similar_songs) == 1:
        ...
        answer = ("Sorry, there is no similar song to '" + songtitle +
                        "' by" + artist + " in this corpus")
    return answer

def query_find_song_about(topic, root):
    """Tries to find songs about this topic in a corpus."
    ...
    Returns:
        ...
        apology if no song about that topic could be found in the corpus.
    """
    songs = []
    for child in root:
...
def query_find_topics_of_artist(artist, root):
    ...
    for child in root:
        ...
        if artist_child == artist:
            ...
            answer = "Sorry, the artist you requested is not in the corpus"
    return answer
